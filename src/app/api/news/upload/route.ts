...
export async function POST(request: NextRequest) {
  try {
    ...
    const filename = `news_${timestamp}.webp`;
    ...
    const storagePath = `public/news/${filename}`;

    // Save to StorageMedia table if newsId is provided and valid
    if (newsId && newsId !== '0') {
      try {
        await prisma.storageMedia.create({
          data: {
            parentId: parseInt(newsId),
            type: 'image',
            parentType: 'news',
            path: storagePath,
          },
