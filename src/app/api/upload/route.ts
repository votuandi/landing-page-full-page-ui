...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get productId from query params or form data
    const { searchParams } = new URL(request.url);
    const productId = searchParams.get('productId') || formData.get('productId') as string;
    ...
    if (isImage) {
      ...
      uploadDir = path.join(process.cwd(), 'public', 'videos', 'products');
      ...
      fileUrl = `/videos/products/${filename}`;
