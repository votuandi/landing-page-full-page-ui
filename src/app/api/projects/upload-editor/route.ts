...
export async function POST(request: NextRequest) {
  try {
    ...
    const projectId = searchParams.get('projectId') || formData.get('projectId') as string;
    ...
    if (isImage) {
      ...
      filename = `project_${timestamp}.webp`;
      ...
      filename = `project_${timestamp}.${extension}`;
      uploadDir = path.join(process.cwd(), 'public', 'videos', 'projects');
      ...
      fileUrl = `/videos/projects/${filename}`;
      filePath = `public/videos/projects/${filename}`;
    }

    // Save to storage_medias table with parent_type = "project-text-editor"
    // For new projects (projectId = 0), save with parentId = null and include the path in response
    // The parent will update these records after the project is created
    try {
      const parentIdValue = projectId && projectId !== '0' ? parseInt(projectId) : null;
